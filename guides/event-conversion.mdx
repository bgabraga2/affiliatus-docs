...
<ParamField body="order_id" type="string" required>
  ...
  **Exemplo:** `"ORDER-12345"`, `"ch_1234abcd"`, `"TXN-2024-001"`
...
<ParamField body="product" type="string">
  ...
  **Exemplo:** `"Plano Premium Anual"`, `"Produto X"`
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Envie APENAS após confirmação" icon="circle-check">
    **Aguarde** webhook ou confirmação do gateway. **Nunca** envie no clique do botão de compra.
    ...
    ```javascript
    // ❌ ERRADO - No clique do botão
    button.addEventListener('click', () => {
      affiliatus.trackConversion({ /* ... */ });
      checkout();
    });
    
    // ✅ CORRETO - No webhook após pagamento confirmado
  ...
  <Accordion title="Use order_value correto" icon="dollar-sign">
    ...
    ```javascript
    // ✅ CORRETO - Valor que o cliente pagou
    order_value: 99.90 // Após desconto de R$ 10
    
    // ❌ ERRADO - Valor antes do desconto
    order_value: 109.90
    
    // ❌ ERRADO - Valor após taxas do gateway
    order_value: 94.90
    ```
  </Accordion>
...
<Steps>
  ...
  <Step title="Aprove manualmente no dashboard">
    ...
    Dashboard → Conversões → Ações → Aprovar
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="Aprovar Conversões" icon="circle-check" href="https://app.affiliatus.io/dashboard/conversions">
    Gerenciar conversões no dashboard
  </Card>
  
  <Card title="Criar Payouts" icon="money-bill" href="https://app.affiliatus.io/dashboard/payouts">
