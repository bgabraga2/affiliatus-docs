...
A integração via API REST permite que você envie conversões diretamente do seu backend, sem depender de JavaScript no frontend. Perfeito para integrações server-side e sistemas legados.

**Quando usar:**
- Você processa conversões no backend
- Usa gateway de pagamento não suportado
- Precisa de controle total sobre quando enviar eventos
...
```mermaid
graph LR
    A[Cliente compra] --> B[Seu backend processa]
    B --> C[Backend envia POST para API]
    C --> D[API valida API key]
    D --> E[Conversão criada automaticamente]
```

1. Cliente finaliza compra no seu sistema
2. Seu backend processa o pagamento
3. **Backend envia conversão via API REST**
4. API valida a API key
5. **Conversão é criada automaticamente** ✨

<Note>Essa integração é totalmente server-side - nenhum código JavaScript no frontend!</Note>
...
// Uso
await sendConversion('ORDER-123', 99.90, 'JOAO1');
```
...
<CardGroup cols={2}>
  <Card title="API Reference" icon="book-open" href="/api-reference/events/send-events">
...
<AccordionGroup>
  ...
  <Accordion icon="question" title="Posso enviar outros tipos de evento além de conversão?">
    Sim! Você pode enviar `page_view` e `lead` também usando o mesmo formato.
  </Accordion>
</AccordionGroup>

<Note>
  **Dúvidas?** Entre em contato pelo suporte ou consulte a [documentação da API](/api-reference/introduction).
</Note>
