...
- Quer automação total sem código adicional
- Aceita pagamentos instantâneos via PIX
- Busca integração nativa com pagamentos brasileiros
...
```mermaid
graph LR
    A[Cliente acessa com ?ref=CODIGO] --> B[Cookie é salvo]
    B --> C[Cliente gera PIX na Woovi]
    C --> D[PIX é pago]
    D --> E[Woovi envia webhook]
    E --> F[Affiliatus cria conversão automaticamente]
```

1. Cliente acessa site com link de afiliado
2. Código do afiliado é passado no `additionalInfo` da cobrança
3. Cliente escaneia QR Code e paga o PIX
4. Woovi confirma o pagamento
5. **Woovi envia webhook para o Affiliatus**
6. **Conversão é criada automaticamente** ✨

<Note>Você não precisa chamar `trackConversion()` manualmente! O webhook faz tudo.</Note>
...
```
https://api.affly.io/api/campaigns/{SEU_ID}/woovi/webhook
```
...
<Tip>A Woovi envia um evento de teste automaticamente. Se retornar sucesso, está tudo certo!</Tip>
...
<Note>A validação de assinatura é opcional mas recomendada para segurança adicional.</Note>
...
<Warning>O campo `affiliate_code` no `additionalInfo` é obrigatório para rastrear o afiliado!</Warning>
...
- ✅ Inclua `affiliate_code` no `additionalInfo`
- ✅ Use `correlationID` único para cada cobrança
- ✅ Monitore os logs de webhook na Woovi
- ✅ Armazene o `chargeId` para referência

### ❌ Não Faça

- ❌ Não exponha seu App ID no frontend
- ❌ Não crie cobranças duplicadas com mesmo `correlationID`
- ❌ Não esqueça de converter valores para centavos
...
<CardGroup cols={2}>
  ...
  <Card title="Dashboard" icon="chart-line" href="/quickstart#passo-7-aprove-conversoes-e-comissoes">
...
<AccordionGroup>
  <Accordion icon="question" title="Funciona com assinaturas recorrentes?">
    Sim! A Woovi suporta cobranças recorrentes. Cada pagamento gera uma nova conversão.
  </Accordion>
  
  <Accordion icon="question" title="O PIX pode expirar antes do pagamento?">
    Sim. Configure o tempo de expiração na criação da cobrança. O padrão é 24 horas.
  </Accordion>
  ...
  <Accordion icon="question" title="Preciso do SDK JavaScript também?">
    Não é obrigatório, mas recomendado para rastrear page views e leads antes da conversão.
  </Accordion>
  
  <Accordion icon="question" title="Posso usar com Mercado Pago ou PagSeguro?">
    No momento, apenas Woovi/OpenPix tem integração nativa. Para outros, use SDK JavaScript.
  </Accordion>
</AccordionGroup>

<Note>
  **Dúvidas?** Entre em contato pelo suporte ou consulte a [documentação da Woovi](https://developers.openpix.com.br/).
</Note>
